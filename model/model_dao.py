...
class ModelDAO:
    ...
    def __init__(self, database, exc_tags=None):    # database : 데이터베이스, exc_tags : 모델에 반영할 데이터 중 제외할 태그 리스트
        self.db = database

        self.rec_model = RecModel()         # 추천 모델 세팅
    ...
    def set_user_data(self, input_data):
        ...
        for i, data in enumerate(input_data):
            userdatas.loc[i] = [data.user_no, data.tag_no, data.tag, data.u_cnt,
                         data.l_cnt, data.s_cnt_, data.s_cnt_w, data.s_cnt_m,
    ...
    def set_recognized_tag_data(self, input_data, exc_tags=None):
        ...
        for data in input_data:
            for i, rec_tag in enumerate(data.rec_tags):
                if rec_tag.tag_no in exc_tags:
                    continue
    ...
    def get_preferences_to_df(self):
        ...
        for i, pf in enumerate(preferences):
    ...
    def set_recommend_images(self):
        ...
        for u in users:
            ...
            _r_imgs = self.rec_model.get_recommend_images(u.user_no, self.user_df, self.tag_df, self.rectag_df, prefer_df)  # 추천 모델을 통해 추천 이미지 생성
